...
def load_lottieurl(url):
    try:
        ...
        if r.status_code != 200:
            st.warning(
                f"Error al cargar la animación Lottie. Código de estado: {r.status_code}")
            return None
        return r.json()
...
with st.sidebar:
    ...
    selected = option_menu(
        ...
        styles={
            ...
            "nav-link": {"font-size": "16px", "text-align": "left", "--hover-color": "#0F3460"},
            "nav-link-selected": {"background-color": "#16213E", "color": "#E94560"},
        }
    )
        ...
        if st.session_state['prediction_result'] is not None:
            ...
            with result_cols[0]:
                ...
                <div style="text-align: center; margin-top: 20px;">
                    ...
                    <div style="font-size: 2.5rem; font-weight: 700; background: linear-gradient(45deg, {species_color}, #6C63FF); -webkit-background-clip: text; -webkit-text-fill-color: transparent;">{species}</div>
                </div>
                """, unsafe_allow_html=True)
...
# Footer
st.markdown("""
<div style="text-align: center; margin-top: 50px; padding: 20px; background-color: rgba(255, 255, 255, 0.7); border-radius: 10px;">
